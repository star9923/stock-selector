...
- **快速**：默认只分析成交量前 200，只跑技术面和实时行情可得的基本面字段，关闭情绪分析和逐股财报接口，并优先复用 1 天内历史 K 线缓存。适合盘中反复刷新。
- **均衡**：分析成交量前 500，开启情绪分析和逐股财报指标，适合常规选股。
- **深度**：扩大样本到成交量前 1000，保留完整情绪/财报分析，适合收盘后复盘。

选股完成后，结果区会显示耗时、分析样本数、行情条数、TopN、并发数、情绪/财报/历史缓存开关，方便判断本次运行为什么快或慢。
...
以下测试不需要真实联网数据源，会 mock 数据源并验证快速选股链路、东方财富批量行情标准化、历史 K 线缓存优先策略：
